...
const int gripper_abre = 3;     // the number of the pushbutton pin
const int gripper_cierra = 0;     // the number of the pushbutton pin
const int brazo_sube =  1;      // the number of the LED pin.
const int brazo_baja =  2;      // the number of the LED pin
...
// Calculamos la funciÃ³n de salida del PID.
...
// Guardamos el valor de algunas variables para el prÃ³ximo ciclo de cÃ¡lculo.
lastInput = Input;
lastInput2 = Input2;
lastInput3 = Input3;
lastInput4 = Input4;
...
void setup() {
  ...
  digitalWrite(brazo_baja, LOW); 
  digitalWrite(brazo_sube, LOW);  
  Serial1.begin(9600);
  //pines motor
  ...
  pinMode(gripper_abre, OUTPUT);      
  pinMode(gripper_cierra, OUTPUT);  
  pinMode(brazo_sube, OUTPUT); 
  pinMode(brazo_baja, OUTPUT); 
...
if(Serial1.available()>0)
  {
    val=Serial1.read();
      ...
      set_point[0]= Serial1.read();
      while(Serial1.available()==0) {}; 
      set_point[1]= Serial1.read();
       while(Serial1.available()==0){};  
      set_point[2]= Serial1.read();
       while(Serial1.available()==0) {}; 
      set_point[3]= Serial1.read();
       while(Serial1.available()==0) {}; 
      set_point[4]= Serial1.read();
       while(Serial1.available()==0) {}; 
      set_point[5]= Serial1.read();
//      while(Serial1.available()==0){};
//        int i=0;
//        for(i=0;i<6;i=i+1){
...
if(set_point[5]>160){
    digitalWrite(brazo_baja, HIGH); 
    digitalWrite(brazo_sube, LOW); 
}
else if(set_point[5]<30){
    digitalWrite(brazo_baja, LOW); 
    digitalWrite(brazo_sube, HIGH); 
}
else{
 digitalWrite(brazo_baja, LOW); 
 digitalWrite(brazo_sube, LOW);   
...
//int timeChange = (now2 - now1); 
//Serial.println((int)timeChange);

//  Serial.print((int)set_point[0]);
//  Serial.print(",");
//  Serial.print((int)set_point[1]);
//  Serial.print(",");
//  Serial.print((int)set_point[2]);
//  Serial.print(",");
//  Serial.print((int)set_point[3]);
//  Serial.print(",");
//  Serial.print((int)set_point[4]);
//  Serial.print(",");
//  Serial.println((int)set_point[5]); 
  
//  delay(20);
//Asignacion de variables
// 255 0 255 0
  if(set_point[0]+set_point[1]==0 || set_point[0]+set_point[1]==255 || set_point[2]+set_point[3]==0 || set_point[2]+set_point[3]==255){
  analogWrite(4,set_point[0]);
  analogWrite(5,set_point[1]);
  analogWrite(8,set_point[0]);//ejecucion de pwms
  analogWrite(9,set_point[1]);  
  //el otro par
  analogWrite(6,set_point[2]);
  analogWrite(7,set_point[3]);
  analogWrite(10,set_point[2]);//ejecucion de pwms
...
//  Serial.println((int)Input4);  
  ...
  if(set_point[0]>set_point[1] && set_point[2]>set_point[3] ){ 
  Setpoint_m1=map(set_point[0], 0, 255, 0, 49);//escalar para maximo 100 equivalente a 255
  Setpoint_m2=map(set_point[2], 0, 255, 0, 58);//escalar para maximo 100 equivalente a 255
  Setpoint_m3=map(set_point[0], 0, 255, 0, 45);//escalar para maximo 100 equivalente a 255
  Setpoint_m4=map(set_point[2], 0, 255, 0, 50);//escalar para maximo 100 equivalente a 255
  ...
  analogWrite(4,m1a);//ejecucion de pwms
  analogWrite(5,0);
  analogWrite(8,m3a);
  analogWrite(9,0);
  
  analogWrite(6,m2a);//ejecucion de pwms
  analogWrite(7,0);
  analogWrite(10,m4a);
  analogWrite(11,0);
 }
  
  else if(set_point[0]>set_point[1] && set_point[2]<set_point[3]) {
  Setpoint_m1=map(set_point[0], 0, 255, 0, 49);//escalar para maximo 100 equivalente a 255
  Setpoint_m2=map(set_point[3], 0, 255, 0, 58);//escalar para maximo 100 equivalente a 255
  Setpoint_m3=map(set_point[0], 0, 255, 0, 45);//escalar para maximo 100 equivalente a 255
  Setpoint_m4=map(set_point[3], 0, 255, 0, 50);//escalar para maximo 100 equivalente a 255
  Compute();
  m1a=Output;
  m2a=Output2;
  m3a=Output3;
  ...
  analogWrite(6,0);//ejecucion de pwms
  analogWrite(7,m2a);
  analogWrite(10,0);
  analogWrite(11,m4a);
  }
  ...
  Setpoint_m1=map(set_point[1], 0, 255, 0, 49);//escalar para maximo 100 equivalente a 255
  Setpoint_m2=map(set_point[2], 0, 255, 0, 58);//escalar para maximo 100 equivalente a 255
  Setpoint_m3=map(set_point[1], 0, 255, 0, 45);//escalar para maximo 100 equivalente a 255
  Setpoint_m4=map(set_point[2], 0, 255, 0, 50);//escalar para maximo 100 equivalente a 255
  ...
  analogWrite(4,0);
  analogWrite(5,m1a);//ejecucion de pwms
  analogWrite(8,0);
  analogWrite(9,m3a);  
